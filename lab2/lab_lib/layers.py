...
class SoftmaxCrossEntropyWithLogits():
  ...
  def forward(self, x, y):
    ...
    # TODO
    exp_x = np.exp(x)
    log_soft_x = -np.log(exp_x / np.sum(exp_x, keepdims=True, axis=1)) * y
    return np.mean(np.sum(log_soft_x, axis=1), axis=0)
  ...
  def backward_inputs(self, x, y):
    ...
    #
    exp_x = np.exp(x)
    soft_x = exp_x / np.sum(exp_x, keepdims=True, axis=1)
    return (soft_x - y) / x.shape[0]
